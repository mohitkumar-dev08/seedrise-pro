...
export default function ChallengePlant({ streak }) {
  const [animate, setAnimate] = useState(false);

  // Streak ke according plant growth
  const getPlantStage = () => {
    if (streak >= 15) return { emoji: "🌳", name: "Full Grown Tree 🌳", stage: 5 };
    ...
    return { emoji: "🌰", name: "Seed 🌰", stage: 1 };
  };
  ...
  return (
    <div className="challenge-plant-card">
      <div className="plant-header">
        ...
        <h4>15 Day Challenge</h4>
        <span className="plant-streak">🔥 Day {streak}</span>
      </div>
      ...
      <div className="plant-progress-bg">
        <div className="plant-progress-bar" style={{ width: `${(streak / 15) * 100}%` }} />
      </div>
      ...
      <div className="plant-milestone">
        {streak >= 15 ? (
          ...
          <span>{15 - streak} days to go!</span>
        )}
      </div>
    </div>
  );
